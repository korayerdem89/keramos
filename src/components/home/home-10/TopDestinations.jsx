      ...
      >
        {destinations10.map((item) => (
          <SwiperSlide key={item.id}>
            <Link href="/tour-list-v2" className="citiesCard -type-3 d-block h-full rounded-4 ">
              ...
              <div className="citiesCard__content px-30 py-30">
                <h4 className="text-26 fw-600 text-white text-capitalize">{item.location}</h4>
                <div className="text-15 text-white">{item.numberOfProperties} properties</div>
              </div>
            </Link>
          </SwiperSlide>
        ))}
